...
import { GESTURES, GestureActions, consumeAction, initialState } from './gestures.js';
...
function renderHistory() {
  ...
  $('history').innerHTML = state.history.length ? state.history.map(item => {
    const g = GESTURES.find(g => g.id === item.gesture);
...
const camera = new GestureCamera({ video,
  onStatus({ state: status, sessionId, source }) {
    ...
    if (status === 'requesting') {
      recognizer = new GestureActions({ sessionId, source }); state = initialState({ sessionId, source });
  ...
  onFrame(frame, inferenceMs) {
    ...
    const g = GESTURES.find(g => g.id === action.action);
    $('gesture-icon').textContent = g?.icon ?? '—';
    $('gesture-name').textContent = g?.name ?? (frame.hands.length > 1 ? 'Show one hand' : hand ? (hand.category === 'None' ? 'Unknown gesture (None)' : 'Uncertain gesture') : 'No hand detected');
    $('confidence').textContent = hand ? `Model confidence ${Math.round(hand.score * 100)}% · ${hand.category.replaceAll('_', ' ')}` : 'Model confidence —';
    ...
    if (next.history[0]?.id !== state.history[0]?.id) {
      ...
      $('feedback').textContent = lastAction.command ? `${lastAction.command === 'Confirm' ? '✓' : '✕'} ${lastAction.command}` : `${g.name} recognized`;
    }
...
window.gestureLab = { getState: () => ({ running: camera.running, active: camera.active,
