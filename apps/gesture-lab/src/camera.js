...
export class GestureCamera {
  constructor({ video, onFrame = () => {}, onStatus = () => {}, onError = () => {}, onStop = () => {} }) {
    this.video = video;
  ...
  async start() {
    ...
    attempt.initializationTimer = setTimeout(() => {
      this.fail(attempt, new Error('Camera or gesture model initialization timed out. Please try again.'));
  ...
  async open(attempt) {
    try {
      ...
      const worker = new Worker(new URL('runtime/gesture-worker.js', base));
      attempt.worker = worker;
      worker.onmessage = ({ data }) => this.receive(attempt, data);
      worker.onerror = () => this.fail(attempt, new Error('The local gesture model worker failed. Please try again.'));
      worker.onmessageerror = () => this.fail(attempt, new Error('The local gesture model returned an unreadable result.'));
  ...
  receive(attempt, data) {
    if (this.attempt !== attempt) return;
    if (data.type === 'error') {
      this.fail(attempt, new Error(data.message || 'The local gesture model failed.'));
    ...
    } else if (data.type === 'hands' && attempt.ready && attempt.pending?.tMs === data.time) {
      const input = attempt.pending;
      attempt.pending = null;
      attempt.lastResultAt = performance.now();
      try {
        // Preserve shared PoseFrame metadata and named wrists. Finger observations
        // are an optional app-local experiment, not a new shared contract.
        const joints = {};
        for (const hand of data.hands) {
          if (hand.side === 'Left') joints.leftWrist = hand.joints.wrist;
          if (hand.side === 'Right') joints.rightWrist = hand.joints.wrist;
        }
        const frame = { version: 1, ...attempt.session, seq: input.seq, tMs: input.tMs,
          modelId: 'mediapipe-gesture-f16-v1/tasks-vision-0.10.32',
          coordinateSpace: 'image-normalized-unmirrored',
          image: { width: input.width, height: input.height }, joints, hands: data.hands };
        assertPoseFrame(frame);
        // Slow results are not evidence of a continuously held gesture.
        if (performance.now() - input.tMs > 250) {
          frame.hands = []; frame.joints = {};
        }
        this.onFrame(frame, data.inferenceMs);
      } catch (error) { this.fail(attempt, error); }
    }
  }
