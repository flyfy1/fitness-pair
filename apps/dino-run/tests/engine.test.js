...
test('calibrated motion height directly follows rise/descent; progress never counts jumps', () => {
  ...
  const input=createRunnerMotionInput(g);input.reset(session); g.command('start');
  let seq = 0;
  ...
  for (const ratio of [.25,.5,1,.5,0]) {
    assert.equal(input.consume(frame(ratio)),true); g.step(.04);
  ...
  input.consume(landing); assert.equal(g.jumps,1);
  input.consume({...landing,inputSeq:++seq,tMs:seq*40}); assert.equal(g.jumps,1);
  input.consume({...frame(.9),sessionId:'foreign'}); assert.equal(g.y,0);
  const stale = frame(.4); input.consume(stale); input.consume({...stale,heightRatio:1}); assert.equal(g.y,.4*MOTION_MAX_HEIGHT);
  g.command('pause'); input.consume(frame(1)); assert.equal(g.y,.4*MOTION_MAX_HEIGHT);
});

test('motion mode rejects uncalibrated and missing poses and resets on control switch', () => {
  const g = new Runner();g.setControlMode('motion');
  const source={kind:'synthetic',id:'test'};const input=createRunnerMotionInput(g);input.reset({sessionId:'test',source});
  ...
  assert.equal(input.consume(frame),false);assert.equal(g.y,0);
