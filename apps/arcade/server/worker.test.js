...
test('consent, type, duration, game and size are checked before reading video',()=>{assert.equal(validateUpload(new Request(url,{headers}),url).source,'synthetic');for(const changes of [{'X-Sharing-Consent':''},{'Content-Type':'text/html'},{'Content-Length':String(21*1024*1024)}])assert.throws(()=>validateUpload(new Request(url,{headers:{...headers,...changes}}),url));for(const [key,value] of [['duration','61'],['game','unknown'],['source','camera'],['title','']]){const changed=new URL(url);changed.searchParams.set(key,value);assert.throws(()=>validateUpload(new Request(changed,{headers}),changed));}});
