...
test('mock GCP publication persists, retries idempotently, streams ranges, and revokes without exposing its key',async()=>{
 ...
 const fetcher=async(raw,options)=>{
  const u=new URL(raw);if(u.hostname==='oauth2.googleapis.com')return Response.json({access_token:'test-token',expires_in:3600});
