...
export function validateUpload(request,url){
 ...
 return {title,source,game,duration,mime,key};
}
...
function publicClip(record){const {id,title,source,game,duration,mime,createdAt,expiresAt}=record;return {id,title,source,game,duration,mime,createdAt,expiresAt,url:'/clips/'+id};}
export function createWorker({fetcher=fetch,now=()=>Date.now()}={}){
 ...
 async function routes(request,env){
  ...
  if(path==='/api/clips'&&request.method==='GET'){
   if(!enabled(env))return json({enabled:false,clips:[]});
  ...
  if(match){
   const [,kind,id]=match;if(!ID.test(id))throw fail(404,'Clip not found.');if(!enabled(env))throw fail(503,'Community publishing is being connected. Your local clips remain on this device.');
   ...
   if(kind==='clips'&&request.method==='PUT'){
    if(!await equal(request.headers.get('Authorization'),'Bearer '+env.SHARE_UPLOAD_CODE))throw fail(401,'Enter a valid early-access upload code.');
    ...
    try{
     try{const existing=await record(env,id);if(!await equal(existing.keyHash,await digest(info.key)))throw fail(409,'This clip ID is already used.');return json(publicClip(existing));}catch(error){if(error.status!==404)throw error;}
     ...
     const upload=await gcp(env,`videos/${id}`,{method:'POST',body,mime:info.mime});if(!upload.ok)throw fail(503,'Upload did not finish. Your local clip is safe; retry shortly.');
     const {key,...metadata}=info;const entry={...metadata,id,createdAt:now(),expiresAt:now()+TTL,keyHash:await digest(key)};
     const marker=await gcp(env,`gallery/${id}.json`,{method:'POST',body:JSON.stringify(entry),mime:'application/json'});
     if(!marker.ok){await gcp(env,`videos/${id}`,{method:'DELETE'}).catch(()=>{});throw fail(503,'Publication did not finish. Your local clip is safe; retry shortly.');}
     return json(publicClip(entry),201);
    }finally{uploadBusy=false;}
   }
   const entry=await record(env,id);
   if(kind==='clips'&&request.method==='DELETE'){
    const key=(request.headers.get('Authorization')||'').replace(/^Bearer /,'');if(!await equal(await digest(key),entry.keyHash))throw fail(403,'Use the device that published this clip to remove it.');
    ...
    await gcp(env,`videos/${id}`,{method:'DELETE'}).catch(()=>{});return json({removed:true});
   }
   if(!['GET','HEAD'].includes(request.method))throw fail(405,'Method not allowed.');
   if(kind==='clips')return json(publicClip(entry));
