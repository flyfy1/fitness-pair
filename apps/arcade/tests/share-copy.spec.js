...
test('long real media produces a bounded unbranded share copy without replacing original',async({page})=>{
 ...
 await expect(page.locator('.clip-card')).toHaveCount(2,{timeout:70000});const copy=page.locator('.clip-card').first();await expect(copy.getByRole('heading',{name:'Motion Quest · share copy',exact:true})).toBeVisible();
