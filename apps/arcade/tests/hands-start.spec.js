...
const games=[...['breakout','invaders','stack','knife','bubble','fruit'].map(id=>({id:'ar-'+id,start:'#start',stop:'#stop'})),{id:'dino-run',start:'#start',stop:'#stop-camera'},{id:'dino-ar',start:'#primary',stop:'#stop'},{id:'motion-quest',start:'#start',stop:'#stop'},{id:'jump-game',start:'#primary',stop:'#stop'}];
for(const config of games)test(`${config.id}: camera play waits for left hand and release with one camera and worker`,async({page},info)=>{
 ...
 expect(await game.locator('.hands-start').evaluate(el=>[getComputedStyle(el).backgroundColor,getComputedStyle(el).backgroundImage])).toEqual(['rgba(0, 0, 0, 0)','none']);await expect(game.locator('.hands-start progress')).toHaveCount(0);if(config.id==='dino-ar')await expect(game.locator('#instruction')).toBeHidden();
 await page.waitForTimeout(3500);await expect(page.locator('#record-panel')).not.toHaveAttribute('data-state','recording');
 await pose({hands:'both'});await page.waitForTimeout(1150);await expect(game.locator('.hands-start-title')).toContainText('LEFT');await pose({hands:'down'});await page.waitForTimeout(500);
 await pose({hands:'left'});await expect(game.locator('.hands-start-title')).toContainText('lower both');await page.waitForTimeout(600);await expect(page.locator('#record-panel')).not.toHaveAttribute('data-state','recording');
