...
import {syntheticCamera as guidedCamera,confirmWithHand} from '../../camera-start/tests/browser/synthetic-camera.js';
const energy=async video=>{await openReplay(video);return video.evaluate(async v=>{const context=new AudioContext();try{const b=await context.decodeAudioData(await(await fetch(v.src)).arrayBuffer());const d=b.getChannelData(0);return Math.sqrt(d.reduce((sum,x)=>sum+x*x,0)/d.length);}finally{await context.close();}});};
