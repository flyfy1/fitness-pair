...
const storedTrackingBytes=clip=>clip.trackingBytes??(clip.tracking?new TextEncoder().encode(JSON.stringify(clip.tracking)).byteLength:0);
export const clipBytes=clip=>clip.blob.size+(clip.conversation?.blob?.size||0)+(clip.thumbnail?.size||0)+storedTrackingBytes(clip);
