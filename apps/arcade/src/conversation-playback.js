...
export function attachConversationPlayback(video,conversation,{enabled,onState=()=>{},onError=()=>{}}){
 ...
 async function sync(){
  ...
  const position=video.currentTime-(conversation.offsetSeconds||0);
  if(source&&rate===video.playbackRate&&Math.abs(startOffset+(context.currentTime-startAt)*rate-position)<.2){gain.gain.value=video.muted?0:video.volume;return;}
  ...
  try{
   ...
   const time=video.currentTime-(conversation.offsetSeconds||0);
   if(time>=buffer.duration)return;
   rate=video.playbackRate;startOffset=Math.max(0,time);startAt=context.currentTime+Math.max(0,-time)/rate;
