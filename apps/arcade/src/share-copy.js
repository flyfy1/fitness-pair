import {CLIP_WIDTH,CLIP_HEIGHT,loadRecordingLogo,drawClipEnding,drawDownloadFrame} from './clip-compositor.js';
...
export async function createShareCopy(clip,{signal,onProgress=()=>{},includeConversation=false,fullLength=false,playbackRate=1,brandedDownload=false}={}){
 ...
 try{
  ...
  const draw=()=>brandedDownload?drawDownloadFrame(ctx,video,clip,logo):ctx.drawImage(video,0,0,CLIP_WIDTH,CLIP_HEIGHT);
  ...
  return {id:crypto.randomUUID(),parentId:clip.id,title:`${clip.gameTitle||clip.title.split(' · ')[0]} · ${fullLength?'with conversation':'share copy'}`,game:clip.game,createdAt:Date.now(),source:clip.source,includesCamera:clip.includesCamera,includesAudio:!!clip.includesAudio||!!voiceBuffer,conversationEmbedded:!!voiceBuffer||!!clip.conversationEmbedded,gameTitle:clip.gameTitle,brand:clip.brand,website:clip.website,shareCopy:!fullLength,branded:brandedDownload||clip.branded!==false,playbackRate:(clip.playbackRate||1)*playbackRate,endingSeconds:brandedDownload?3:fullLength?(clip.endingSeconds??3)/playbackRate:0,hasEnding:brandedDownload||(fullLength&&!!clip.hasEnding),finalScore:clip.finalScore,...encoded};
 }finally{
