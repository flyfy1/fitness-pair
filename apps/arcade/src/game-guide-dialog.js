...
export function installGameGuides(container,games){
 ...
 for(const link of container.querySelectorAll('a[href^="/play/"]'))link.setAttribute('aria-haspopup','dialog');
 container.addEventListener('click',event=>{
  ...
  dialog.innerHTML=`<div class="guide-heading"><div><p class="guide-eyebrow">HOW TO PLAY · ${game.kind==='concept'?'POINTER PREVIEW':'MOVEMENT GAME'}</p><h2 id="guide-title">${game.title}</h2><p class="guide-goal">${guide.goal}</p></div><button class="guide-close" aria-label="Close instructions" autofocus>×</button></div>
  ...
  <div class="guide-footer"><p>${game.kind==='concept'?'No camera needed.':'Camera starts only when you enable it inside the game. Gameplay and camera replay stay on this device unless you choose to share.'}</p><a class="button primary" href="/play/${game.id}">Let’s play <span aria-hidden="true">→</span></a></div>`;
