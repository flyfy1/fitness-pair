...
export const expiryLabel = expiresAt => expiresAt === null ? 'Never expires' : 'Expires ' + new Date(expiresAt).toLocaleDateString(languageTag(readLanguage()));
export const expiryMarkup = expiresAt => expiresAt === null ? 'Never expires' : `Expires <time datetime="${new Date(expiresAt).toISOString()}">${new Date(expiresAt).toLocaleDateString(languageTag(readLanguage()))}</time>`;
...
export async function renderShared(container, notice = '') {
  ...
  try {
    ...
    content.innerHTML = `<section class="storage-summary" aria-label="Shared storage"><div><strong>${storageLabel(account.usedBytes)} of 2 GB used</strong><span>${storageLabel(Math.max(0, account.limitBytes - account.usedBytes))} available</span></div><meter min="0" max="${account.limitBytes}" value="${account.usedBytes}" aria-label="Shared storage used"></meter><p>Deleting clips frees space. Each video keeps the expiry selected when sharing. Permanent videos use storage until you remove them. Each clip can be up to 90 seconds / 200 MB.</p></section><div class="clip-grid" data-owned-clips></div>`;
    ...
    for (const clip of account.clips) {
      ...
      if(!clip.unavailable)mountSharedPreview(card,clip);
      grid.append(card);
    }
  } catch (error) {
