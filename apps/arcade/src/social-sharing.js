...
export function shareMessage(clip,{origin=SITE_URL,local=false}={}){
 ...
 const intro=clip.source==='synthetic'?`Check out my ${game?.title||'game'} demo on ${BRAND_NAME}!`:`I played ${game?.title||'a game'} on ${BRAND_NAME}!`;
 if(local)return {url:gameURL,text:`${intro}\nI'm sending you my video as an attachment.\nTry the game: ${gameURL}`,intro};
 ...
 const expiry=Number.isFinite(clip.expiresAt)?`\nAvailable until ${new Date(clip.expiresAt).toISOString().slice(0,10)} (UTC).`:'';
 const privacy=clip.visibility==='private'?'\nThis is a private link. Anyone with it can watch; please keep it between us.':'';
