...
export function mountGame(container,game){
 ...
 container.innerHTML=`<main id="main" class="game-play" aria-label="${game.title}">
  ...
  <section class="local-result" id="local-result" hidden></section><div id="game-feedback-root"></div></main>`;
 ...
 function controls(){
  ...
  disposeControls=mountConversationControls(doc,element=>{recorder.connectControls(element);feedback.connectControls(element);});
 }
 frame.addEventListener('load',controls);if(frame.contentDocument?.readyState==='complete')controls();
 let disposed=false;
 function dispose(){if(disposed)return;disposed=true;frame.removeEventListener('load',controls);frame.removeEventListener('load',reload);disposeControls();feedback.dispose();recorder.dispose();runtime.dispose();window.removeEventListener('pagehide',dispose);frame.src='about:blank';document.body.classList.remove('game-mode');}
