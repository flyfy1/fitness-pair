...
export function mountConversationControls(doc,connect){
 ...
 .hopmodo-conversation span{position:absolute;right:0;top:calc(100% + 6px);width:max-content;max-width:min(260px,calc(100vw - 24px));padding:8px;border:1px solid #18234655;border-radius:8px;background:#fff;line-height:1.4}
 ...
 .hopmodo-native-hud{padding-right:calc(var(--hopmodo-native-padding,0px) + 48px)!important;gap:min(12px,2vw)!important;box-sizing:border-box}
 ...
 const place=()=>{(doc.fullscreenElement||doc.body).append(element);};place();
 doc.addEventListener('fullscreenchange',place);connect(element);
 return ()=>{doc.removeEventListener('fullscreenchange',place);hud?.classList.remove('hopmodo-native-hud');element.remove();style.remove();};
}
