export async function syntheticCamera(page) {
  await page.addInitScript(() => {
    // Reproducible enemy/fruit choices; movement and collisions remain real.
    Math.random = () => window.gameRandom ?? .01;
    window.poseTest = { x: 0, aimX: .56, aimY: .6, rise: 0, hand: 'down', missing: false, wristsMissing: false, delay: 0, crouch: false, noiseFrames: 0 };
    ...
    window.Worker = class {
      ...
      postMessage(data) {
        ...
        if (!s.missing) {
          for (const [indices,x] of [[[11,23],.44],[[12,24],.56]]) indices.forEach((id,i)=>{points[id]={x:x+s.x,y:[.28,.52][i]-s.rise,visibility:.99};});
          if (s.crouch) {
