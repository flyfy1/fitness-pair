...
test('large tutorial requires real recognized practice and keeps game physics stopped',async({page},info)=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await practice(page);const frozen=(await state(page)).game;expect(await page.locator('#tutorial-title').evaluate(el=>parseFloat(getComputedStyle(el).fontSize))).toBeGreaterThanOrEqual(90);
...
test('skip before permission opens normal calibration and never opens the camera itself',async({page},info)=>{
 ...
 await page.locator('#tutorial-skip').click();expect(await page.evaluate(()=>!!window.testStream)).toBe(false);await expect(page.locator('#start')).toBeVisible();await page.locator('#start').click();await startWithHands(page);await expect.poll(async()=>(await state(page)).phase).toBe('playing');await page.locator('#finish').click();
});
test('missing tracking cannot advance practice; mid-practice skip still waits for tracking',async({page})=>{
 ...
 await move(page,{missing:false,x:0});await startWithHands(page);await expect.poll(async()=>(await state(page)).phase).toBe('playing');await page.locator('#finish').click();
});
