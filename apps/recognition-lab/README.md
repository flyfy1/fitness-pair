...
sample-to-result delay, build revision and browser identity. This delay is not an end-to-end motion latency metric.
...
Select a session, inspect the skeleton next to its timeline, scrub or play at original timing, and compare **Recorded** with
...
Five unit checks cover squat/jump replay, calibration prefixes, missing joints,
annotation failures and input validation. Thirteen production Chrome checks cover
capture/autosave/reload/export/import, manual assertions, issue markers, playback,
storage failure, camera denial/cancellation/stalls/hidden tabs, fullscreen review, desktop/mobile AR
geometry and a real local MediaPipe run on the public `mediapipe-assets/pose.jpg`.
Screenshots were inspected at desktop and 390px widths. Browser test capture data
is generated by synthetic streams or that public image and is not human evidence.
No private participant skeletons have been collected as part of these checks.
